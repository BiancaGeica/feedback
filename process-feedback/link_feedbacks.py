...
def main():
    ...
    for c in courses:
        ...
        if f_id == None:
            print("No feedback for course {} ({}).".format(c['shortname'], c['id']))
            continue
        short = c['shortname'].replace("/","|")
        print("Linking feedback id {} to name {}.".format(f_id, short))
        try:
            os.remove(os.path.join(args.outdir, "{}.json".format(short)))
        except OSError:
            pass
        os.link(os.path.join(feedback_contents, "{}.json".format(f_id)), os.path.join(args.outdir, "{}.json".format(short)))
